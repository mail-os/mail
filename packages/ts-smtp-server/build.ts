...
// The Zig project root is two levels up from packages/ts-smtp-server/
...
function build(options: BuildOptions = {}): void {
  ...
  const nativeBinary = join(zigProjectRoot, 'zig-out', 'bin', 'smtp-server')
  if (existsSync(nativeBinary)) {
    const platform = process.platform === 'darwin' ? 'macos' : 'linux'
    const arch = process.arch === 'arm64' ? 'aarch64' : 'x86_64'
    const dest = join(binDir, `smtp-server-${arch}-${platform}`)
...
function buildTarget(target: string, optimize: string, label: string): void {
  try {
    ...
    const possiblePaths = [
      join(zigProjectRoot, 'zig-out', 'bin', label, `smtp-server-${label}`),
      join(zigProjectRoot, 'zig-out', 'bin', `smtp-server-${label}`),
      join(zigProjectRoot, 'zig-out', 'bin', 'smtp-server'),
    ]

    for (const src of possiblePaths) {
      if (existsSync(src)) {
        const dest = join(binDir, `smtp-server-${label}`)
