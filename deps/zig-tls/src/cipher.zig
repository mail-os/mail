...
/// Comptime-known Random using ChaCha CSPRNG with lazy seeding
var lazy_csprng: ?std.Random.ChaCha = null;
var lazy_csprng_initialized: bool = false;

fn lazyRandomFill(_: *anyopaque, buf: []u8) void {
    if (!lazy_csprng_initialized) {
        var seed: [32]u8 = undefined;
        // Seed from stack address and pointer entropy
        const addr1: usize = @intFromPtr(&lazy_csprng);
        const addr2: usize = @intFromPtr(&seed);
        @memcpy(seed[0..@sizeOf(usize)], std.mem.asBytes(&addr1));
        @memcpy(seed[@sizeOf(usize)..][0..@sizeOf(usize)], std.mem.asBytes(&addr2));
        // Use libc clock for additional entropy
        var ts: std.c.timespec = undefined;
        _ = std.c.clock_gettime(.REALTIME, &ts);
        @memcpy(seed[16..24], std.mem.asBytes(&ts.sec));
        @memcpy(seed[24..32], std.mem.asBytes(&ts.nsec));
        lazy_csprng = std.Random.ChaCha.init(seed);
        lazy_csprng_initialized = true;
    }
    lazy_csprng.?.fill(buf);
}

pub const io_random = std.Random{
    .ptr = undefined,
    .fillFn = &lazyRandomFill,
};
